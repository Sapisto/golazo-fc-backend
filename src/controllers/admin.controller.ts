...
export const createPlayer = async (
    ...
    try {
        ...
        const team = await Team.findOne({ where: { name: { [Op.iLike]: teamName } } });
        if (!team) {
        ...
        await sendEmailWithRetry(
            () => sendPlayerInviteEmail(email, rawPassword, firstName, username),
            3,
            3000
        );
        ...
        return res.status(201).json({
            ...
            data: {
                player,
                loginDetails: { email: email, password: rawPassword },
            },
        });
    } catch (error: any) {
        await t.rollback();
